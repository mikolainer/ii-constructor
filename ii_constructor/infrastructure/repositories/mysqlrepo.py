...
class SourceMySQL(Source):
    ...
    def __init__(self, conn: pymysql.Connection, id: ScenarioID) -> None:
        ...
        cur = conn.cursor()
        cur.execute("SELECT name, description FROM projects WHERE id=%s", (id.value,))
        conn.commit()
    ...
    def save_lay(self, id: StateID, x: float, y: float):
        self.__do("UPDATE `states` SET `x` = %s, `y` = %s WHERE `states`.`project_id` = %s AND `states`.`id` = %s", (x, y, self.id.value, id.value))

    def delete_state(self, state_id:StateID):
        self.__do("DELETE FROM `states` WHERE `project_id` = %s AND `id` = %s", (self.id.value, state_id.value))
    ...
    def set_answer(self, state_id:StateID, data:Output):
        self.__do("UPDATE `states` SET `answer` = %s WHERE `states`.`project_id` = %s AND `states`.`id` = %s", (data.value.text, self.id.value, state_id.value))
    ...
    def select_vectors(self, names:Optional[list[Name]] = None) -> list['InputDescription']:
        ...
        if names == []:
            ...
            cur.execute("SELECT DISTINCT `name` FROM `vectors` WHERE `project_id` = %s", (self.id.value,))
            conn.commit()
        ...
        for _name in _names:
            ...
            cur.execute("SELECT `value` FROM `synonyms` WHERE `project_id` = %s AND `group_name` = %s", (self.id.value, _name.value))
            conn.commit()
    ...
    def get_vector(self, name:Name) -> InputDescription:
        ...
        conn: pymysql.Connection = self.__db_connection
        cur = conn.cursor()
        cur.execute("SELECT `value` FROM `synonyms` WHERE `project_id` = %s AND `group_name` = %s", (self.id.value, n_val))
        conn.commit()
    ...
    def add_vector(self, input: InputDescription):
        ...
        self.__do("INSERT INTO `vectors` (`name`, `type`, `project_id`) VALUES (%s, %s, %s)", (_name, _type, self.id.value))

        # создать синонимы
        for synonym in input.synonyms.synonyms:
            self.__do("INSERT INTO `synonyms` (`group_name`, `value`, `project_id`) VALUES (%s, %s, %s)", (_name, synonym.value, self.id.value))

    def remove_vector(self, name:Name):
        self.__do("DELETE FROM `vectors` WHERE `vectors`.`project_id` = %s AND `vectors`.`name` = %s", (self.id.value, name.value))
    ...
    def check_vector_exists(self, name:Name) -> bool:
        ...
        conn: pymysql.Connection = self.__db_connection
        cur = conn.cursor()
        cur.execute("SELECT EXISTS(SELECT 1 FROM `vectors` WHERE `project_id` = %s AND `name` = %s) as ans", (self.id.value, name.value))
        conn.commit()
        (answer,) = cur.fetchone()
        return answer
    ...
    def set_synonym_value(self, input_name: str, old_synonym: str, new_synonym: str):
        self.__do("UPDATE `synonyms` SET `value`= %s WHERE `project_id`= %s AND `group_name`= %s AND `value` = %s", (new_synonym, self.id.value, input_name, old_synonym))
            
    def create_synonym(self, input_name: str, new_synonym: str):
        self.__do("INSERT INTO `synonyms` (`project_id`, `group_name`, `value`) VALUES (%s, %s, %s)", (self.id.value, input_name, new_synonym))
            
    def remove_synonym(self, input_name: str, synonym: str):
        self.__do("DELETE FROM `synonyms` WHERE `project_id` = %s AND `group_name` = %s AND `value` = %s", (self.id.value, input_name, synonym))
    
    def rename_state(self, state:StateID, name:Name):
        self.__do("UPDATE `states` SET `name`= %s WHERE `project_id`= %s AND `id`= %s", (name.value, self.id.value, state.value))

    def rename_vector(self, old_name:Name, new_name: Name):
        self.__do("UPDATE `vectors` SET `name`= %s WHERE `project_id`= %s AND `name`= %s", (new_name.value, self.id.value, old_name.value))

class HostingMySQL(Hosting):
    __connection: Optional[pymysql.Connection]
