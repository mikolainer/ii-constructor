...
from PySide6.QtGui import QContextMenuEvent, QMouseEvent, QPainter
...
class FlowsDelegate(QStyledItemDelegate):
    ...
    def __on_slider_visible_changed(
        self,
        visible: bool,
        index: QModelIndex,
        editor: FlowWidget,
    ):
        index.model().setData(index, visible, CustomDataRole.SliderVisability)
        editor.adjustSize()
    ...
    def __remove_row(self, index: QModelIndex):
        if not self.model().removeRow(index.row()):
            QMessageBox.warning(
                ...
                "Невозможно удалить точку входа. Эта команда должна быть реализована для яндекс диалогов!",
            )
