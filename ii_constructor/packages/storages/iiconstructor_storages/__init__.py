...
from iiconstructor_storages.presentation import StorageConnectionViewModel, StoragePluginViewModel, StorageConnectionPresenter, StoragePluginPresenter
...
class StoragePluginController:
    ...
    def read(self) -> set[StorageConnectionViewModel]:
        return {self.__conn_presenter.present(conn) for conn in self.__manager.read()}

    def connect(self, conn: StorageConnectionViewModel, login: str, password: str):
    ...
    def disconnect(self, conn: StorageConnectionViewModel):
    ...
    def invalidate_plugin(self, plugin:StoragePluginViewModel) -> StoragePluginViewModel:
    ...
    def invalidate_connection(self, conn:StorageConnectionViewModel) -> StorageConnectionViewModel:
